...
int main(int argc, char **argv)
{
    ...
    int grid_size = n % block_size == 0 ? n / block_size : n / block_size + 1;  // grid size
    ...
    dim3 gpu_grid(grid_size, grid_size);    // set up the grid dimensions
    ...
    if (kernel >= GPU_KERNEL_END)
        printf("GPU kernel version: cuBLAS\n");
    ...
    for (size_t i = 0; i < NR_ITER; ++i) {
        if (kernel < GPU_KERNEL_END) {
           ...
           cublasSgemv(handle, CUBLAS_OP_N, n, n, &alpha, gpu_A, n, gpu_x, 1, &beta, gpu_y, 1);
        }
#ifdef _DEBUG_
        cudaError_t err;
