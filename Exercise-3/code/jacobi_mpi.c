...
inline void Jacobi(double ** u_previous, double ** u_current, int X_min, int X_max, int Y_min, int Y_max) {
    int i, j;
...
int main(int argc, char ** argv) {
    ...
    */
    ...
    for (t=0;t<T;t++) {
        ...
        req=0;
        if (north >= 0) {
            MPI_Irecv(&u_previous[0][1], j_max, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        if (west >= 0) {
            MPI_Irecv(&u_previous[1][0], 1, vertical_border, west, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        if (south >= 0) {
            MPI_Irecv(&u_previous[ local[0]+1 ][1], j_max, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        if (east >= 0) {
            MPI_Irecv(&u_previous[1][ local[1]+1 ], 1, vertical_border, east, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        ...
        req = 0;
        if (north >= 0) {
            MPI_Isend(&u_previous[1][1], j_max, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
        if (west >= 0) {
            MPI_Isend(&u_previous[1][1], 1, vertical_border, west, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
        if (south >= 0) {
            MPI_Isend(&u_previous[ local[0] ][1], j_max, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
        if (east >= 0) {
            MPI_Isend(&u_previous[1][ local[1] ], 1, vertical_border, east, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
    ...
    //----Printing results----//
    if (rank==0) {
        printf("Jacobi X %d Y %d Px %d Py %d Iter %d ComputationTime %lf ConvergeTime %lf TotalTime %lf midpoint %lf\n", global[0],
