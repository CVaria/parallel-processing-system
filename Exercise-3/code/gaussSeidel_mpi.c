...
int main(int argc, char ** argv) {
    ...
    struct timeval tts, ttf, tcs, tcf;   //Timers: total-tts,ttf, computation-tcs,tcf, converge-cts,ctf
    ...
    */
    ...
    for (t=0;t<T;t++) {
        ...
        req=0;
        if (north >= 0) {
            MPI_Irecv(&u_current[0][1], j_max, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        if (west >= 0) {
            MPI_Irecv(&u_current[1][0], 1, vertical_border, west, t, MPI_COMM_WORLD, &r_request[req]);
            req++;
        }
        ...
        req2=0;
        if (south >= 0) {
            MPI_Irecv(&u_current[ local[0]+1 ][1], j_max, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &r_request[req2]);
            req2++;
        }
        if (east >= 0) {
            MPI_Irecv(&u_current[1][ local[1]+1 ], 1, vertical_border, east, t, MPI_COMM_WORLD, &r_request[req2]);
            req2++;
        }
        ...
        req=0;
        if (north >= 0) {
            MPI_Isend(&u_current[1][1], j_max, MPI_DOUBLE, north, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
        if (west >= 0) {
            MPI_Isend(&u_current[1][1], 1, vertical_border, west, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }

        if (south >= 0) {
            MPI_Isend(&u_current[ local[0] ][1], j_max, MPI_DOUBLE, south, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
        if (east >= 0) {
            MPI_Isend(&u_current[1][ local[1] ], 1, vertical_border, east, t, MPI_COMM_WORLD, &s_request[req]);
            req++;
        }
    ...
    //----Printing results----//
    if (rank==0) {
        printf("GaussSeidel X %d Y %d Px %d Py %d Iter %d ComputationTime %lf ConvergeTime %lf TotalTime %lf midpoint %lf\n", global[0],
